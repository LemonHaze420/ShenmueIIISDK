...
public:
	...
	float GetNearStartPointDist(struct FTransform* Point, int* poiIdx, int* stIdx);
	void CalcZLocation();
	bool IsCharacterSitting(class AS3Character** Character);
	bool ModeChangeRequest(EChairOccupied* Mode, class AS3Character** Character, struct FVector* pos, int* poiIdx, int* stIdx, float* moveDist, struct FS3ChairResultStruct* Result);
