...
public:
	...
	void STATIC_InitSet(const struct FDataTableRowHandle& EventId, int UseFlagID, bool LoadStart);
	void UserConstructionScript();
	void STATIC_ReceiveTick(float DeltaSeconds);
	void RenewalActorCount(TEnumAsByte<EN_MainFlowActorID> ActorId, unsigned char RenewalCounter);
	void STATIC_KillSheLead();
	void STATIC_StartLeadSHE_Macro(bool LoadStart);
	void STATIC_ExecuteUbergraph_BP_Hakkason_leadShe(int EntryPoint);
};
