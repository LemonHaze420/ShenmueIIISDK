...
public:
	...
	void PrestreamIfNotOther(class UActorComponent* Target, bool bEnableStreaming, class UActorComponent* Other);
	void SetNextComponent(class US3AttentionComponent* TargetComp);
	void Prestream(class UActorComponent* Target, bool bEnableStreaming);
	void UpdateButton();
	void ResetLevelState();
	void IsStartAC(bool* StartAC);
	void OnAttentionLevelUnloaded(bool* NeetFade);
	void UpdateVisibleActorControl();
	void SetActorVisibleControl(bool Condition);
	void CheckCameraHit(const struct FVector& MoveVec, bool* bHit);
	void CheckDirection(class US3AttentionComponent* Component, bool* bFind);
	...
	void IsBackerACEvent(bool* Return);
	bool IsInputActionEnabled();
	bool IsEffectEnd();
	void EndAttention();
	void SetInputActionEnabled(bool Enabled);
	void NotificationEvent(struct FString* ScriptLabel);
	void CalcFrontVector(struct FVector* FrontVec);
	void SetAdjacentComponent(class US3AttentionComponent* AdjacentComponent);
	void TraceRotateTarget(TEnumAsByte<ECarouselRotate> Direction);
	void SetAttentionComponent(class US3AttentionComponent* AttentionComponent, class US3AttentionComponent** Output);
	void UpdateCursor();
	void ResetEffectEndFlag();
	void DrawAttentionEffect();
	void SetPlayerVisibility(bool Visibility);
	void UpdateZoomRate(float* CameraZoomOffset);
	...
	bool CapsuleTrace(const struct FVector& Start, const struct FVector& End, float Radius, float HalfHeight, TArray<class AActor*>* ignoreActors, class UPrimitiveComponent** HitComponent);
	void ResetCamera(float BlendTime);
	void ExitAttentionRequest();
	void ExitAttention();
	void PlayGetSE();
	void InputAction(ES3ActionIconType Type);
	void IsLookActor(const struct FVector& StartTrace, const struct FVector& EndTrace, bool* Look);
	void SearchReset();
	void StartAttention(class US3AttentionComponent* InAttentionComponent, const struct FVector& InHitLocation);
	void SearchTarget();
	void FreeMove(float DeltaSeconds);
	void CheckNeighbourTarget(class US3AttentionComponent* Component);
	void DrawArrow(const struct FVector& Vec);
	void DrawNextDirection();
	void InputKey(const struct FName& KeyName);
	void CheckCondition(int* Result);
	void CameraSetting();
	void MoveCamera();
	void ChangeTarget();
	void CalcNormal(class US3AttentionComponent* Component, const struct FVector& HitLocation, struct FVector* Normal);
	void TraceNextTarget(class US3AttentionComponent* Component, const struct FName& KeyName, bool Add, class US3AttentionComponent** NextComponent);
	void PlayScript(const struct FString& ScriptLabel, bool DisableButton, bool Once, bool* Playing);
	...
	void DelayStartAttention();
	void DelayEndAttention();
	void AxisToDirection();
	...
	void OnExitAttentin__DelegateSignature();
	void OnEndAttention__DelegateSignature();
	void OnStartAttention__DelegateSignature();
	void OnFindNPC__DelegateSignature(class ABP_ScheduleNPC_C* NPC);
	void OnAccordCondition__DelegateSignature(const struct FString& EventId);
};
