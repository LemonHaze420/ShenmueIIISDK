...
public:
	...
	void STATIC_RotateUpdate();
	float ClacArmPower(float Value);
	void STATIC_GetMiniGameBGMID(struct FName* ID);
	TArray<struct FS3DetectActionParam> GetDetectAction();
	void SetBlockingVolume(bool Enabled);
	void GetMiniGameHelpName(struct FString* String);
	void SetBoardInfo();
	void InitializeArrow();
	void STATIC_SetAndInterpHandRotate(bool IsInterp, float Target, float DeltaTime, float InterpSpeed);
	bool IsChangePowerAngle();
	float STATIC_GetInputPowerLength();
	void STATIC_AddItem(int* ItemCount);
	void STATIC_AddGiftList(const struct FName& GiftName);
	void STATIC_UpdateArrow(float DeltaSeconds);
	void UpdateShaking(float DeltaSeconds);
	void InitializeValue();
	void STATIC_CameraUpdate(float DeltaSeconds);
	void SetRightAxisY(float Value);
	void SetRightAxisX(float Value);
	void HideUI();
	class ABP_MiniGameInputBase_C* STATIC_SpawnInputControlActor();
	void STATIC_ShowUIThrowResult(bool IsSuccess, int Point, bool IsVisible);
	void ShowUIGameResult();
	void STATIC_SetUIScore(int InScore);
	void STATIC_SetUIThrowCount(int Count);
